...
async def auto_filter(bot, update):
    ...
    for i in Translation.EXPELLED :
       if i in the_query.split() :
          for a in Translation.IN_EXPELLED :
    ...
    for i in Translation.KITTO.split() :
    ...
    for i in "#admins #admin #complaint #report @admin @admins".split():
      if i in the_query.lower() :
         await update.forward(chat_id=Translation.OWNER_ID)
         await bot.send_message(
             chat_id=update.chat.id,
    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            if not Translation.SIZE_BUTTON:
    ...
    if len(results) == 0 : # double check
        ...
        result[0].append([
            InlineKeyboardButton("♻️ Help ♻️", callback_data="help_popup")
        ])
        ...
        try:
            ...
            await bot.send_photo(
                photo=movie_url,
                caption=f"<b>📀Movie : </b> <code>{query}</code> \n <b>🗃️Files :</b> <code>{len_results if len_results <= max_pages*8 else max_pages*8}</code>",
                reply_markup=reply_markup,
                chat_id=update.chat.id,
                reply_to_message_id=update.message_id,
                parse_mode="html"
